...
export interface ImportSummary { status: string; import_job_id: number; filename: string; rows_processed: number; new_components: number; updated_components: number; unchanged_components: number; invalid_rows: number; unmatched_rows: number; invalid_details: string[]; flagged_matches: string[]; low_stock_components: number; }
